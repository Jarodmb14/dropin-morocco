...
BEGIN
  IF EXISTS (
    ...
    -- Create function to update location from coordinates
    ...
    RAISE NOTICE 'Created location update trigger';
  ELSE
    RAISE NOTICE 'No location column found - skipping trigger creation';
  END IF;
END $$;

-- Step 8: Create PostGIS functions (only if location column exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    RAISE NOTICE 'Created PostGIS functions';
  ELSE
    RAISE NOTICE 'No location column found - skipping PostGIS function creation';
  END IF;
END $$;
...
-- Step 12: Test the functions (only if they exist)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.routines 
    WHERE routine_name = 'find_nearby_clubs'
  ...
  ELSE
    RAISE NOTICE 'PostGIS functions not created - location column may not exist';
  END IF;
END $$;
