-- Drop-In Morocco: Fixed Database Schema Updates for Business Rules
...
-- 7. FIXED: Update user roles enum safely
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (
        ...
        -- Add PARTNER to existing enum if it doesn't exist
        ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'PARTNER';
    END IF;
    
    -- No need to recreate the entire enum, just add the missing value
END $$;
...
-- Only create partner policy if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'orders' 
        AND policyname = 'Partners can view their venue orders'
    ) THEN
        EXECUTE 'CREATE POLICY "Partners can view their venue orders" ON orders FOR SELECT 
        USING (
            EXISTS (
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE 'CREATE POLICY "Admins can view all orders" ON orders FOR ALL 
        USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = ''ADMIN''))';
    END IF;
END $$;
...
-- Success message
SELECT 'Drop-In Morocco database schema updated successfully! 🎉' as result;
