...
-- CASABLANCA (8 clubs)
('FitZone Casablanca', 'Modern fitness center in the heart of Casablanca with state-of-the-art equipment and professional trainers', 'STANDARD', 'Casablanca', 'Boulevard Mohammed V, Casablanca', 33.5731, -7.5898, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'group_classes']), '+212522123456', 'info@fitzone-casa.ma', true, '00000000-0000-0000-0000-000000000001'),

('Premium Gym Casa', 'Luxury fitness experience with personal trainers, spa services, and premium amenities in the heart of Maarif', 'PREMIUM', 'Casablanca', 'Maarif, Casablanca', 33.5792, -7.6187, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'spa', 'personal_training', 'pool']), '+212522234567', 'contact@premiumgym.ma', true, '00000000-0000-0000-0000-000000000002'),

('Basic Fitness Center', 'Affordable gym with essential equipment and friendly atmosphere for everyday fitness', 'BASIC', 'Casablanca', 'Derb Ghallef, Casablanca', 33.5650, -7.6034, to_jsonb(to_jsonb(ARRAY['cardio', 'weights']), '+212522345678', 'info@basicfitness.ma', true, '00000000-0000-0000-0000-000000000003'),

('Elite Sports Club', 'High-end sports facility with tennis courts, swimming pool, and comprehensive fitness programs', 'LUXURY', 'Casablanca', 'Anfa, Casablanca', 33.5589, -7.6678, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'tennis', 'swimming', 'spa', 'personal_training']), '+212522456789', 'info@elitesports.ma', true, '00000000-0000-0000-0000-000000000004'),
...
('City Gym Rabat', 'Modern gym in downtown Rabat with group classes and personal training options', 'STANDARD', 'Rabat', 'Hassan, Rabat', 34.0134, -6.8480, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'group_classes']), '+212537234567', 'contact@citygym.ma', true, '00000000-0000-0000-0000-000000000010'),
...
('Diplomatic Club', 'Exclusive gym for diplomatic community with international standards', 'LUXURY', 'Rabat', 'Souissi, Rabat', 34.0123, -6.8345, to_jsonb(ARRAY['cardio', 'weights', 'spa', 'personal_training', 'international_programs']), '+212537678901', 'contact@diplomaticclub.ma', true, '00000000-0000-0000-0000-000000000014'),

-- MARRAKECH (6 clubs)
('Atlas Fitness Marrakech', 'Modern gym with mountain views and comprehensive fitness programs', 'STANDARD', 'Marrakech', 'Gueliz, Marrakech', 31.6348, -8.0099, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'group_classes']), '+212524234567', 'info@atlasfitness.ma', true, '00000000-0000-0000-0000-000000000015'),

('Medina Gym', 'Traditional gym in the heart of old Marrakech with authentic atmosphere', 'BASIC', 'Marrakech', 'Medina, Marrakech', 31.6295, -7.9811, to_jsonb(to_jsonb(ARRAY['cardio', 'weights']), '+212524345678', 'contact@medinagym.ma', true, '00000000-0000-0000-0000-000000000016'),

('Palace Fitness', 'Luxury gym in historic palace with traditional architecture and modern equipment', 'LUXURY', 'Marrakech', 'Hivernage, Marrakech', 31.6234, -7.9956, to_jsonb(ARRAY['cardio', 'weights', 'spa', 'personal_training', 'palace_atmosphere']), '+212524456789', 'info@palacefitness.ma', true, '00000000-0000-0000-0000-000000000017'),
...
('Resort Fitness', 'Luxury resort-style gym with spa services and vacation packages', 'LUXURY', 'Agadir', 'Resort Area, Agadir', 30.4123, -9.5890, to_jsonb(ARRAY['cardio', 'weights', 'spa', 'personal_training', 'resort_amenities']), '+212528456789', 'info@resortfitness.ma', true, '00000000-0000-0000-0000-000000000028'),

-- FEZ (4 clubs)
('Imperial Fitness Fez', 'Traditional gym in the imperial city with modern equipment', 'STANDARD', 'Fez', 'Fes el-Bali, Fez', 34.0331, -5.0003, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'group_classes']), '+212535123456', 'info@imperialfitness.ma', true, '00000000-0000-0000-0000-000000000029'),

('Luxury Gym Fez', 'High-end fitness center with premium amenities including spa and sauna', 'LUXURY', 'Fez', 'Fes el-Jdid, Fez', 34.0528, -4.9844, to_jsonb(ARRAY['cardio', 'weights', 'spa', 'personal_training', 'pool', 'sauna']), '+212535234567', 'contact@luxurygym.ma', true, '00000000-0000-0000-0000-000000000030'),
...
-- MEKNES (2 clubs)
('Royal City Fitness', 'Fitness center in the royal city with historic atmosphere and modern equipment', 'STANDARD', 'Meknes', 'Centre Ville, Meknes', 33.8935, -5.5473, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'group_classes']), '+212555123456', 'info@royalcityfitness.ma', true, '00000000-0000-0000-0000-000000000033'),
...
-- OUJDA (2 clubs)
('Eastern Fitness', 'Gym in eastern Morocco with modern facilities and regional programs', 'STANDARD', 'Oujda', 'Centre Ville, Oujda', 34.6814, -1.9086, to_jsonb(to_jsonb(ARRAY['cardio', 'weights', 'group_classes']), '+212536123456', 'info@easternfitness.ma', true, '00000000-0000-0000-0000-000000000035'),
